...
describe("Blog setup", () => {
  ...
  test("the default value of likes is 0 if the likes property is missing", async () => {
    ...
    assert.strictEqual(response.length, helper.initialBlogs.length + 1);
    const createdBlog = response.find(
      (blog) => blog.title === "Homework" && blog.author === "Abbas Kiarostami",
    );
  ...
  test("a blog can be updated", async () => {
    ...
    const likesAfterUpdating = await responseAfterUpdating.map(
      (blog) => blog.likes,
    );
